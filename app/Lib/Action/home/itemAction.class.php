...
class itemAction extends frontendAction {
    ...
    public function fetch_item() {
		...
		//库中是否存在
        $item_mod = D('item');
        include_once(LIB_PATH . 'Pinlib/itemcollect/taobao/taobao_itemcollect.class.php');
        $taobao_itemcollect = new taobao_itemcollect;
		$key_id = $taobao_itemcollect->get_key($url);
		$exists_item = $item_mod->where(array('key_id'=>$key_id))->find();
		if($exists_item) {
			$info = array('item'=>$exists_item);
		} else {
			//获取商品信息
